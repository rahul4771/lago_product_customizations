...
import { Icon, Spinner, Autocomplete, TextContainer, Stack, Tag, InlineError,Frame,Toast } from '@shopify/polaris'
...
const EditSalesrep = (props) => {
  ...
  const salesRepId = props.params ? JSON.parse(props.params) : '';
  ...
  useEffect(() => {
    router.prefetch("/header?tab=sales-reps&page=salesList");
    try {
  ...
  const addSalesRep = async () => {
    ...
    if (updateSalesRep && updateSalesRep.message == 'success') {
      setLoading(false);
      setToggleMessage("Successfully created a sales rep - "+firstName+" "+lastName);
      settoggleActiveT(true);
      setFirstName("");
      setLastName("");
      setEmail("");
      setExistSelectedOptions([]);
      setSelectedOptions([]);
      setTimeout(function () {
      ...
      setLoading(false);
      setActive(true);
      setErrorMessage("Sales Rep create failed")
    }
  };
                  ...
                  <div className="Polaris-Card__Section min--height">
                    <div className="Polaris-FormLayout">
                      <div role="group" className="Polaris-FormLayout--grouped">
                        ...
                        <div className="display-text">
                          <div className="one-half text-right" style={{paddingLeft:'89%'}}>
                            <button
